...
function BuyTicket(props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setMessage(
        <div className="mt-5">
          <h3 className="text-2xl font-bold text-cyan-600 text-center">
            {purchasedTickets.length} Purchased Tickets
          </h3>
          <h3 className="text-xl font-bold text-cyan-600 text-center">
            Thanks !
          </h3>
        </div>
      );
      ...
      {!showSuccess ? (
        <>
          ...
          <div className="text-sm font-bold">
            Select how many tickets would you like to buy?
          </div>
          ...
          {formsData.map((form, index) => (
            <div key={index}>
              ...
              <input
                className="input input-bordered input-info w-full input-md mb-2"
                type="text"
                name="firstName"
        ...
        <>
          {message}
          <div className="mt-5">
            <button className="btn btn-info m-5" onClick={handleKeepLooking}>
              <Link href="/"> I want to keep looking for events</Link>
            </button>
            <Link href="/user_profile">See my tickets</Link>
          </div>
        </>
      )}
    </div>
